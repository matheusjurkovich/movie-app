...
export default function BestMovies() {
    ...
    try {
      const getMovies = async () => {
        const data = await axios.get(
          `https://api.themoviedb.org/3/movie/top_rated?api_key=${apiKey}&language=pt-BR&page=${activePage}`,
          {
            headers: {
  ...
  return (
    <>
      <Header />
      <main className="grid grid-cols-5 gap-6 p-6 dark:bg-zinc-800 max-md:grid-cols-2 ">
        {movies.map((movie) => (
